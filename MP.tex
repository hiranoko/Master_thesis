...
\end{center}
\section*{はじめに}
高エネルギー加速器研究機構 (KEK)では蓄積リング型放射光源 (Photon Factory:PF)が運転されており、蓄積リングを周回する電子ビームから発生する光を使って物性・生命の構造から機能発現の仕組みの研究が行われている。蓄積リングを周回する電子ビームは、ガス散乱や量子寿命、ビーム内散乱によって時間と共に失われてゆく。そのため、定時的に入射器 (LINAC)から供給されるビームを輸送路 (Beam Transport:BT)を通して注ぎ足している。輸送路からリングへの電子ビームの受け渡しを入射と呼ぶ。入射では輸送におけるビームロスを最小化することが要求される。この要求を満たすため、PFではキッカーとセプタムのパルス電磁石を用いた入射方式を採用している。

セプタム電磁石は輸送路の終端部に設置され、入射ビームのみの軌道の向きを曲げて、蓄積ビームの設計軌道と揃えるために用いられる。入射ビームの向きだけを曲げ、蓄積ビームに影響を与えないようにするため、セプタム壁で輸送路とリングを区切り、渦電流の効果を利用して磁場を遮蔽している。キッカー電磁石は入射点にバンプを形成するため、入射点より上流に2台、下流に2台が通常設置される。上流のキッカー電磁石は、リングを周回するビームを入射路側へ寄せて入射振動の振幅を小さくする働きをする。下流のキッカー電磁石は上流のキッカー電磁石と組み合わせて閉じたバンプを形成するように調整される。入射ビームは、セプタム壁を挟んだバンプされた蓄積ビームとの距離を初期振幅として、振動しながらリングを周回する。振動は放射減衰によって決まる時間のスケールで減衰していき、最終的には基準軌道と交じり合って捕獲される。
...
\section*{本研究の目的}
KEK-PFでは、長年の運転で積み重なった誤差の影響と東日本大震災の影響によって、入射効率の低下が問題となっていた。入射効率の低下はビームロスによる放射線の発生につながる。また誤差の影響は蓄積ビームの振動にもあらわれて、放射光強度の変動を誘起し放射光実験の障害となっていた。そこで本研究は、入射効率の改善を目的に、入射状況を改善するため入射パラメータの測定とそれらの入射最適化を実施した。
...
加速器は多数の部品で構成される複雑なシステムであり、恒常的に設置誤差や磁場誤差、挿入光源、フリンジフィールドなど多くの誤差を含む。また、キッカー入射の場合、バンプ内に含まれる6極電磁石を由来とする非線形磁場やパルス電磁石の渦電流の効果、個体差など考慮すべき誤差が追加される。これらの誤差の影響を個別に調べて補正することは困難である。そこで、全ての誤差の影響を含んで返されるビームの応答を元に入射状況を調べる手法を採用した。測定は長直線部両端のBPM電極にビーム位置検出回路を接続して、ドリフトスペースを挟んだ2点の位置情報から直線部中心の位相空間情報を計算し、上流に転送する方法で行った。

    \begin{figure}[H]
        \centering
        \includegraphics[width=100mm]{./figure/00_Foreword/scheme_BBM.png}
        \caption{ビームベース測定の概要図}
    \end{figure}

キッカー電磁石のパルス波形の測定は、キッカーバンプを形成するために必要なパラメータである。測定は、蓄積ビームのシングルバンチを使ってパルスタイミングを変えながら測定を行って、パルス波形に再構築し、結果を得た(付録. 信号処理)。キッカー電磁石の応答は、独立の磁場強度とタイミングの制御を持っている。磁場強度の制御は加速器設置前に行われる実験室での磁場測定の結果を元に設定される。この制御では加速器設置後の特性変化は調べられないため、全てのキッカー電磁石の磁場強度を揃えることはできない。また、全てのキッカーの電磁石の特性は製作精度の困難から揃っておらず、実際にはパルス波形の違いとして現れる。タイミングの制御は、オシロスコープでパルス波形のピーク位置に揃えてLINACからのトリガに合わせて設定される。この制御ではトリガとのタイミングがずれる効果とキッカーケーブル長の違いで個別にタイミングがずれる効果を考慮しないといけない。その他にも6極電磁石の非線形磁場やフリンジフィールドの影響など考慮すべき誤差がある。これら全ての影響を調べる唯一の方法はビームベース測定である。しかし、通常のビーム位置モニターの機能ではパルス波形の再構築に必要な時間方向の情報を得ることができない。そこでビーム位置モニターの処理は利用せず、電極から返される生の電圧の時間波形から信号処理を行うことで再構築に必要な時間のラベルを付与して信号処理を行う、解析プログラムを開発した。その操作によって測定を成功させて、現状のキッカー電磁石のパルス波形をビームの応答から得た。この結果は実際の蹴り角とモニターの蹴り角の補正や加速器設置前後の特性変化の測定できることを示した。
...
入射ビームの位相空間情報とセプタム電磁石とビーム輸送路の補正電磁石の応答の測定は入射調整のために必要なパラメータである。測定は、入射ビームのバンチを使った測定となり、蓄積ビームの測定と比較して電荷量は小さい。また、LINACから直接入射を行う構成のため、バンチごとのばらつきが大きい。そのためノイズに埋もれて信号が取得できなかったり、再現性が得られなかったりと通常は測定されないパラメータである。また電極の配置は基準軌道の補正用に最適化されているため、入射ビームのような大振幅のビームが返す非線形応答の処理がされていない。そこで、ビーム位置モニターの電極から返される生の電圧波形に例外処理を加え、非線形応答へのフィッティングを行った。電極から返される電圧は電荷量と距離に比例した値であり、全ての電極から返される1周期の振幅量を抽出することでビームの電荷量を取得できる。この振幅量に制限を加えて安定した電荷量のデータのみで測定に使用した。非線形応答へのフィッティングはワイヤー校正によるマッピングプロットを使って、解析プログラムに組み込んだ。これらの結果、入射点での位相空間プロットを得て、入射ビームの位相空間情報とセプタム電磁石の応答、ビーム輸送路の応答を調べることができた。入射ビームの位相空間情報の測定では、セプタム壁の位置とビームの位置関係を明らかにした。補正電磁石の応答測定では、入射点から輸送路上流に遡るほど振動の応答は大きくなるため、非線形応答に埋もれて全てのデータをまとめることはできなかった。しかし、入射点でのビームの位相空間プロットとセプタム電磁石の応答を得ることができた。このビーム輸送路終端部に設置された電磁石の応答を調べることは難しく他施設でも報告されていない測定を成功させたことを表す。また、これまでシミュレーションでしかオプティックスマッチングされていない現状に、実ビームを使った調整が可能である将来性を示した。
...
\section*{入射シミュレーション}
つぎに、誤差の影響を見積もるため、入射パラメータの測定結果を用いて入射シミュレーションを行った(第5章)。シミュレーションではバンプ軌道の計算と入射効率の見積もりを実施した。シミュレーションに新しく取り入れた内容は、マルチターンキックの影響と挿入光源によって歪んだビームプティックスの影響である。

バンプ軌道の計算は、蹴り角の組み合わせを解析的に求める方法と蓄積ビームの振動量を最小化して求める方法の2通りをまとめた。はじめに理想的な状況のシングルターンキックをまとめ、つぎに測定したパルス波形を使ってマルチターンキックへの展開を行った。シングルターンキックまでの蹴り角の組み合わせは解析的に求められるが、マルチターンキックへと展開するとき発散する。そこで蓄積ビームの振動量からバンプ軌道を評価する方法をまとめた。

BPMで観測される情報はビームの重心位置である。PFではリングを280個のマルチバンチが周回しており、それぞれの振幅を平均した値が実際に観測される。これを再現するため周期構造への展開を考えた。周期構造への展開は、設定値のタイミング前後の半周期までの蹴り角の比率を単粒子追跡に導入して、蓄積ビームの振動量を求め、平均化する操作である。そこで求めた蓄積ビームの振動量を評価値とした。この再現性は実際のビームでも確認した。
...
入射効率の見積もりでは、必要な位相空間調整と測定したビームオプティックの測定結果を用いる。オプティックの測定はLOCOを使用した(付録. オプティックスの測定)。LOCOとは補正電磁石の応答行列からオプティックスを軌道フィッティングによって得る手法である。応答行列の解析に必要な軌道フィッティングはKEKの加速器シミュレーションコードSADで自作した。それらの操作によって、入射効率の計算に必要なオプティックスとキッカーの蹴り角、生成粒子の位相空間情報を実際に測定したパラメータで準備した。多粒子追跡の結果、現在の入射効率を再現しているモデルを得た。

    \begin{figure}[H]
        \centering
        \includegraphics[width=100mm]{./figure/00_Foreword/injeff_mul03.png}
        \caption{多粒子追跡によって100ターン後も生き残る粒子数。カラーバーのレンジは入射効率(生存粒子数/生成粒子数=1000)を表す。横軸のK3とK4は下流のキッカーの蹴り角の組み合わせを表す。入射効率をよくする方向は、蓄積ビームの振動を犠牲にしたキッカージャンプの効果を表す。現在の入射効率はLINACばらつきを考慮して5割前後で揺らいでおり、現在の入射状況を理解できる。}
    \end{figure}

\section*{入射最適化}
最後に、蓄積ビームの振動を抑制するため、ビームベース最適化を行った(6章)。加速器は多くの誤差を含むモデルである。入射パラメータの測定とシミュレーションでは原因と考えた誤差を検討して、ビームの応答と比較を行い、再現性を確かめた。しかし、考慮していない誤差の影響はある。ビームベース最適化は、直接ビームの応答をみて調整し、誤差の影響を含めた最適化を行うので採用した。ビームベース最適化とは評価関数を決め、パラメータの調整を行い、ビームの応答を直接調べ最適化する手法である。パラメータ調整は自動化のプログラムを独自に作成した。適切な評価関数とパラメータ範囲、測定方法、アルゴリズムなどが設定されて、はじめてパラメータ間の相互作用を無視して最適化できる。ここで大きな課題は応答に含まれるノイズ処理と発散しないための初期設定である。加速器施設における自動調整は目的に合わせて必要な調整方法が各施設で開発されてきた。そのためPFの目的に沿った、独自のプログラムを開発した。

ノイズの影響を受けにくい自動調整にするため、評価関数は蓄積ビームの振動量とした。これは入射シミュレーションにて正しく動作することを確認している。また測定に使用するBPM数を確保でき、シミュレーションと最適化の結果を比較して差異を調べるために選択した。初期パラメータは既に入射効率と蓄積ビームの振動量を考慮したパラメータである運転時のパラメータとした。最適化は発散させることなく自動調整で振動抑制を達成した。またシミュレーション結果は最適化結果を十分に予想できることを示した。
