...
入射率の見積もりでは、必要な位相空間調整と測定したビームオプティックの測定結果を用いる。オプティックの測定はLOCOを使用した。LOCOとは補正電磁石の応答行列からオプティックスを軌道フィッティングによって得る手法である。応答行列の解析に必要な軌道フィッティングはKEKの加速器シミュレーションコードSADで自作した。それらの操作によって、入射率の計算に必要なオプティックスとキッカーの蹴り角、生成粒子の位相空間情報を実際に測定したパラメータで準備した。多粒子追跡の結果、現在の入射率を再現しているモデルを得た。
...
第4章では、入射パラメータの測定を記述する。ここで測定した入射パラメータは、入射点のビーム位置関係とキッカー、セプタム電磁石の特性である。測定方法では、電子ビームを直接測定するビームベース測定で入射パラメータを得るための手順を示す。測定結果では、入射パラメータの結果を現在の状況の比較し、現実の誤差を含んだ応答はどのように影響を与えているのかについてまとめる。

第5章では、入射シミュレーションを記述する。入射シミュレーションでは、キッカーバンプの計算と入射率の計算を行う。ここでは従来の理想的なシングルターンキック入射の手順を示した後、問題としているマルチターンキック入射が及ぼす影響を入射シミュレーションで再現させるために行った計算についてまとめる。また、現実の入射シミュレーションに近づけるため加速器パラメータを測定した。
...
で行われる。規格化座標で図\ref{Off-axis}の入射を考える。
     \begin{figure}[H]
        \centering
        \includegraphics[width=70mm]{./figure/03_Theory_Injection/accep_exam.png}
...
で表される。このとき初期振幅$A_{inj}^2$は図\ref{Off-axis}で、実線の半径に相当する。図\ref{Off-axis2}に、入射されたビームがリングを一周してセプタム壁にあたり失われる様子を示す。
     \begin{figure}[H]
        ...
        \label{Off-axis2}
    \end{figure}
アクセプタンス内に収めるだけであればパルス電磁石のダイポールで振幅を減らすだけでよい。ダイポールマグネットで角度$\theta$蹴り込み、振動を抑制させた様子を図\ref{Off-axis3}と図\ref{Off_axis4}にプロットした。
     \begin{figure}[H]
        \centering
    ...
    \end{figure}
     \begin{figure}[H]
        \centering
        \includegraphics[width=70mm]{./figure/03_Theory_Injection/accep_exam3.png}
        \caption{入射ビームがダイポールマグネットで蹴られた後の入射振動(位相空間)}
        \label{Off-axis4}
    \end{figure}
蹴り込まれたビームはリング内に入射され、内側の実円の上を回転しながら、放射減衰して定常状態のビームの大きさになる。既に蓄積されたビームが存在する場合、ダイポールキックは蓄積ビームの振動をけり振動を起こす。
     \begin{figure}[H]
        \centering
        ...
        \label{Off-axis4}
    \end{figure}
蓄積ビームに影響を与えないないで入射するため、通常の放射光施設では次節のキッカー入射がされる。
%*******************************************************************************
\section{キッカー入射}
     \begin{figure}[H]
        \centering
...
図\ref{Phase_space_kicker_inj}では、入射の振幅をセプタム電磁壁より小さくした。他の部分がよりアクセプタンスを制限するときは、その部分で当たらないように振幅を小さくする。アクセプタンスを制限する可能性のあるコンポーネントとして、放射光から保護するために挿入されるアブソーバを考えた(表\ref{Aperture_phy})。
...
設定値から振幅や励磁タイミングに含まれる誤差の原因として制御系の誤差が考えられる。蹴り角の制御はリング設置前に磁場測定を行い、測定した励磁曲線を用いる。これは実験室測定によって決定したパラメータである。しかしビームベースアライメントしないと、十分な精度とならない場合がある\cite{injection_nsls}。またパルスの立ち上がり時間や立ち下がり時間、波形の不揃いによるミスマッチや渦電流、振幅のZero-to-Zero以降に含まれるアンダーシュートなどが蹴り角の誤差に含まれる。ここでミスマッチは設定値の蹴り角の比率からずれた値を表す。
...
\section{信号処理}
ここでは電極から返される生の電圧の時間波形をビーム位置に変化する手順とパルス波形に再構築する手順の説明する。初めにBPMの測定原理をまとめる。式の導出はOHOセミナーの資料\cite{Kuboki}を参照した。次に生の電圧の時間波形をビーム位置に変換するための解析手順をまとめる。最後にパルス波形の再構築するための解析手順をまとめる。
%*******************************************************************************
\subsection{測定原理}
...
\subsection{入射点でのビーム位置関係}
入射ビームの位置の基準となる蓄積ビームの中心軌道の測定は3 mAのシングルバンチで行った。BPMからの振動は電極からの生信号を解析して、ビーム位置として求めている。
...
図\ref{PS_KK_re}よりパルス波形より蹴り角を揃え、タイミングを実際のビームの挙動から整列することができた。また、パルス幅 (Zero-to-Zero)がキッカーによって異なるため原理的にキッカーバンプマッチングを取れないことが確認できる。K1の測定結果はID02からの位相進みが振幅の節となるため、ID01から転送した結果を採用した。さらに精度の良い結果を得るためには、位相進みが振動の腹となる直線部で測定する必要がある。次に、実際にリングからキッカー電磁石K3インストール前に行われた磁場測定したパルス波形と比較を行った(図\ref{Com_KK})。
...
図\ref{Com_KK}では立ち上がりから1500 nsecまでパルス波形は一致しているが、設定振幅の蹴り角が反対方向にはみ出すアンダーシュート以降で違うことがわかる。この差異は磁場測定だけでは観測されない、あらゆる誤差を含んだ現実の加速器を対象とする、リング内の電磁石から制御系までの環境を含んだ効果が入っている。この結果を用いたシミュレーションを次章で示す。
...
本章では、入射パラメータの測定の章において求めたパラメータを使用し、入射率の改善と蓄積ビームの振動の抑制という入射要件を満たしたパラメータの組み合わせを計算する。入射率は加速器シミュレーションコードSADで\cite{SAD}、多粒子追跡を行い評価した。蓄積ビームの振動は、キッカー電磁石の蹴り角に偏差を導入して生じるベータトロン振動の大きさ(キッカーバンプマッチング)で評価した。作成したモデルの妥当性はビームスタディの結果と比較して調べる。
%*******************************************************************************
\section{キッカーバンプ}
ビームベース測定で得られた入射パラメータを使用して入射のシミュレーションを行う。はじめに理想的なシングルターン入射の計算を考える。通常の入射でキッカー電磁石は、上流2台(K1, K2)と下流2台(K3, K4)の4台のパルス電磁石を用いて局所バンプを形成している。ここでバンプの計算を簡単にするためK1、K3、K4の3台でバンプを形成する。K2は不調で使われていない。K1の蹴り角を決めると、バンプが閉じるK3、K4の蹴り角の組み合わせは一意に決まる。上流のキッカー電磁石に求められる要件は、入射ビームをアクセプタンス内に収めるためにセプタム壁の近くまで寄せることであった。バンプハイトを決めると上流の蹴り角が決まる。ここでバンプハイトを17 \si{mm}とするK1の蹴り角を求め、K3、K4の蹴り角は調整していないときの軌道を図\ref{bump_height}にプロットする。17 \si{mm}はアブソーバにあたらない、蓄積ビームが安定的に削られない最大蹴り角である。
...
図\ref{single_bump}のバンプの形状が通常のシングルターン入射で使用される。この計算は全てのキッカー電磁石でパルス波形が一致し、設定値の蹴り角に揃えられ、励磁タイミングが同期されていることが仮定される。しかし、入射パラメータの測定結果からキッカー電磁石のパルス波形ではパルス幅は異なり、アンダーシュートを含むため、複数回ビームを蹴っていることが予想された。これを考慮したバンプを形成するためのマルチターンキック入射の場合を考える。2ターン目のキッカー電磁石の蹴り角は測定したパルス波形を多項式フィットしてシミュレーションに導入した(図\ref{PS_KK})。また、単純のためパルスタイミングは入射の瞬間にピーク位置で同期され、1ターン目と2ターン目の蹴り角でマルチターンキックを考える。1ターン目のキッカーは .1、2ターン目のキッカーの蹴り角は .2ラベルした。バンプ軌道をシングルターン入射と同じ蹴り角で行い、蓄積ビームの振動を2周目まで追いかけて計算を行った結果を図\ref{bump_second}に示す。
