...
\usepackage{Mp} % Create a cover page
...
\end{eqnarray}
と書ける。ここで、$\varepsilon_0$と$\varphi_0$は初期条件により決定される定数である。J
は次の不変量を表す。
\begin{equation}
...
以下の図\ref{Phase_space}に、クーランシュナイダー不変量から決まる位相空間における軌跡とある粒子が1周期後にどこに来るのか示した写像を示す。左図の楕円のプロットで振幅の最大値はベータトロン振動に相当し、角度の最大値はJの作用に相当する意味を持つ。
